...
/**
 * 个人解题思路：
 *      先将数字转为字符串，然后通过遍历该字符串对应的字符数组，判断首位和末位是否一致，如果一致，则代表是回文数，如果不是，则不是回文数字
 *
 */
public class Main {
    ...
    public static void main(String[] args) {
        ...
        boolean palindrome = solution.isPalindrome(198891);
...
class Solution {
    public boolean isPalindrome(int x) {
        ...
        for(int i = 0; i < chsLength / 2; i++){
            if(chs[i] == chs[chsLength - i - 1]){
                continue;
            }
            else{
                return false;
